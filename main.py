...
TOKEN = "5753791367:AAGlew1KHy8Dz_MeiRUBTCpUMSSrSZ_g36Q"  # Token BotFather
...
#Admins
groupID = "-1001849066365"  ##GroupID
UserNameBot = "@testingunblur_bot"  # User Name Bot
#                           URLS
...
def chegg(update, context):
    ...
    if text.startswith(
        ...
        if int(pi[0]) <= 0 or int(pi[1]) <= 0:
            ...
            Downloader.sd(text, "Dear", update, context, 1, 1, 1,
                          "Answer.html")
...
def echo(URL, name, update, context, fechaCad, cred, fila,
    ...
    if os.path.exists('Answer.html'):
        ...
        remove("Answer.html")
        #requests.post(f'https://api.telegram.org/bot{TOKEN}/sendMessage', data={'chat_id': f'{grup_id}', 'text': f'I will be available in 0 Seconds. ⏱️'})
        time.sleep(0)
